...
nma_side <- function(s = "s", y = "y", v = "v", trt = "trt", comp, ref = "sym", 
  ...
  if(ref == "1"){
    w_p <- mxPath(from = "w", to = paste0("y", coi), free = F, values = c(0, 1))
  }else if(ref == "2"){
    w_p <- mxPath(from = "w", to = paste0("y", coi), free = F, values = c(-1, 0))
  }else if(ref == "sym"){
    w_p <- mxPath(from = "w", to = paste0("y", coi), free = F, values = c(-0.5, 0.5))
  }
