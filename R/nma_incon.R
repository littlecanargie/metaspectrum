...
#' 
#' @seealso \code{\link{nma}}, \code{\link{nma_side}}
#' 
#' @references
#' Shih ...
#' Title.
...
nma_incon <- function(s = "s", y = "y", v = "v", trt = "trt", comp, 
  ...
  v <- mxMatrix(type = "Full", nrow = 2, ncol = 1, free = FALSE, 
                labels = paste0("data.v", coi), name = "v")
  a <- mxMatrix(type = "Full", nrow = 2, ncol = 1, free = FALSE, 
                values = c(-1, 1), name = "a")
  ...
  w_p <- mxPath(from = "w", to = paste0("y", coi), free = FALSE, labels = c("p[1,1]", "p[2,1]"))
